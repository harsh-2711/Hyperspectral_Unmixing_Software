...
int main(int argc, char *argv[])//(double *image, int lines, int samples, int bands, int targets, double SNR, double *endmembers)
{
	...
	dgemm_("T", "N", &bands, &bands, &lines_samples, &alpha, R_o, &lines_samples, R_o, &lines_samples, &beta, svdMat, &bands);
    ...
    dgemm_("N", "T", &targets, &lines_samples, &bands, &alpha, Ud, &targets, R_o, &lines_samples, &beta, x_p, &targets);
	...
	if(SNR < SNR_th)
	{
		...
		dgemm_("T", "N", &bands, &lines_samples, &targets, &alpha, Ud, &targets, x_p, &targets, &beta, Rp, &bands);
	...
	else
	{
		...
		dgemm_("T", "N", &bands, &bands, &lines_samples, &alpha, image_vector, &lines_samples, image_vector, &lines_samples, &beta, svdMat, &bands);
		...
		dgemm_("N", "T", &targets, &lines_samples, &bands, &alpha, Ud, &targets, image_vector, &lines_samples, &beta, x_p, &targets);
		dgemm_("T", "N", &bands, &lines_samples, &targets, &alpha, Ud, &targets, x_p, &targets, &beta, Rp, &bands);
	...
	printf("Iterative VCA: %f segundos", (double)(end - start) / CLOCKS_PER_SEC);
	fflush(stdout);
	//**************************************************
