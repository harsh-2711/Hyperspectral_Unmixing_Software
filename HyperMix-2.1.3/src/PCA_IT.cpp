 ...
 * Centre: Universidad de Extremadura
 * */
int main(int argc, char* argv[])// (double *image_vector, int targets, int lines, int samples, int lines_samples, int bands)
{
	// Variables
	...
	if(argc != 4)
	{
		printf("EXECUTION ERROR PCA Iterative: Parameters are not correct.");
	...
	if(error != 0)
	{
		printf("EXECUTION ERROR PCA Iterative: Error reading header file: %s.", header_filename);
		fflush(stdout);
		exit(-1);
	}
	...
	if(error != 0)
	{
		printf("EXECUTION ERROR PCA Iterative: Error reading image file: %s.", argv[1]);
		fflush(stdout);
		exit(-1);
	}
	...
	else
	{
		...
		if(targets == -1)
		{
			printf("EXECUTION ERROR PCA Iterative: Targets was not set correctly file: %s.", argv[2]);
			fflush(stdout);
			exit(-1);
		}
	}
	...
	dgemm_("T", "N", &bands, &bands, &lines_samples, &alpha, h_X, &lines_samples, h_X, &lines_samples, &beta, h_X2, &bands);
    ...
    dgemm_("N", "N", &lines_samples, &targets, &bands, &alpha, image_vector, &lines_samples, VT, &bands, &beta, imageReduced, &lines_samples);

	//END CLOCK*****************************************
	end = clock();
	printf("Iterative PCA: %f segundos", (double)(end - start) / CLOCKS_PER_SEC);
	fflush(stdout);
	//**************************************************
	...
	if(error != 0)
	{
		printf("EXECUTION ERROR PCA Iterative: Error writing header file: %s.", header_filename);
		fflush(stdout);
		exit(-1);
	}
	error = writeResult(imageReduced,argv[3], lines, samples, targets, dataType, interleave);
	if(error != 0)
	{
		printf("EXECUTION ERROR PCA Iterative: Error writing image file: %s.", argv[3]);
		fflush(stdout);
		exit(-1);
	}
