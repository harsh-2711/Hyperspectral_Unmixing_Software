...
double* LSU(double* endmembers, double* image, int lines, int samples, int bands, int targets)
{
	...
	cudaMalloc((void**)&endmembersD,lines*targets*bands*sizeof(double));
	cudaMalloc((void**)&Et_ED,targets*targets*sizeof(double));
	cudaMemcpy((void*)endmembersD, endmembers, lines*targets*bands*sizeof(double), cudaMemcpyHostToDevice);

	cublasDgemm('T', 'N', targets, targets, bands, alpha, endmembersD, bands, endmembersD, bands, beta, Et_ED, targets);
	...
	double *COMPUTD;
	cudaMalloc((void**)&COMPUTD,targets*bands*sizeof(double));

	cudaMemcpy((void*)Et_ED, Et_E, targets*targets*sizeof(double), cudaMemcpyHostToDevice);

	cublasDgemm('N', 'T', targets, bands, targets, alpha, Et_ED, targets, endmembersD, bands, beta, COMPUTD, targets);
...
int main(int argc, char* argv[])
{
	...
	if(error != 0)
	{
		printf("EXECUTION ERROR IEA Parallel: Error reading header file: %s.", header_filename);
		fflush(stdout);
		exit(-1);
	}
	...
	if(error != 0)
	{
		printf("EXECUTION ERROR IEA Parallel: Error reading image file: %s.", argv[1]);
		fflush(stdout);
		exit(-1);
	}
	...
	else
	{
		...
		if(targets == -1)
		{
			printf("EXECUTION ERROR IEA Parallel: Targets was not set correctly file: %s.", argv[2]);
			fflush(stdout);
			exit(-1);
		}
	}
	...
	double* meanVectorD;
	double* abundancesD;
	double* reconstructedD;

	cudaMalloc((void**)&meanVectorD,bands*sizeof(double));
	cudaMalloc((void**)&abundancesD,lines*samples*sizeof(double));
	cudaMalloc((void**)&reconstructedD,lines*samples*bands*sizeof(double));

	cudaMemcpy((void*)meanVectorD, meanVector, bands*sizeof(double), cudaMemcpyHostToDevice);
	cudaMemcpy((void*)abundancesD, abundances, lines*samples*sizeof(double), cudaMemcpyHostToDevice);

	cublasDgemm('N', 'N', bands, lines_samples, one, alpha, meanVectorD, bands, abundancesD, one, beta, reconstructedD, bands);

	cudaMemcpy((void*)reconstructed, reconstructedD, lines*samples*bands*sizeof(double), cudaMemcpyDeviceToHost);

	cudaFree(abundancesD);
	cudaFree(meanVectorD);
	...
	for(k=2; k<=targets; k++)
	{
		...
		//**********PARALLEL LSU*****************************************
		// E^t * E
		Et_E = (double*)malloc(k*k*sizeof(double));

		cudaMalloc((void**)&endmembersD,lines*k*bands*sizeof(double));
		cudaMalloc((void**)&Et_ED,k*k*sizeof(double));

		cudaMemcpy((void*)endmembersD, endmembers, bands*k*sizeof(double), cudaMemcpyHostToDevice);

		cublasDgemm('T', 'N', k, k, bands, alpha, endmembersD, bands, endmembersD, bands, beta, Et_ED, k);

		cudaMemcpy((void*)Et_E, Et_ED, k*k*sizeof(double), cudaMemcpyDeviceToHost);
		...
		int lwork = k;
		double *work = (double*)malloc(lwork*sizeof(double));
		dgetrf_(&k,&k,Et_E,&k,ipiv, &info);
		dgetri_(&k, Et_E, &k, ipiv,work, &lwork, &info);

		//inv * E^t
		double *COMPUT = (double*)malloc(k*bands*sizeof(double));
		double *COMPUTD;
		cudaMalloc((void**)&COMPUTD,k*bands*sizeof(double));

		cudaMemcpy((void*)Et_ED, Et_E, k*k*sizeof(double), cudaMemcpyHostToDevice);

		cublasDgemm('N', 'T', k, bands, k, alpha, Et_ED, k, endmembersD, bands, beta, COMPUTD, k);

		cudaFree(Et_ED);

		// COMPUT * image

		cudaMalloc((void**)&abundancesD,lines*samples*k*sizeof(double));

		cudaMemcpy((void*)imageD, image_vector, lines*samples*bands*sizeof(double), cudaMemcpyHostToDevice);

		cublasDgemm('N','T',lines_samples, targets, bands, alpha, imageD, lines_samples, COMPUTD, targets, beta, abundancesD, lines_samples);

		//*************************************************************

		cublasDgemm('N', 'N', bands, lines_samples, k, alpha, endmembersD, bands, abundancesD, k, beta, reconstructedD, bands);

		cudaMemcpy((void*)reconstructed, reconstructedD, lines*samples*bands*sizeof(double), cudaMemcpyDeviceToHost);
		cudaMemcpy((void*)image_vector, imageD, lines*samples*bands*sizeof(double), cudaMemcpyDeviceToHost);
	...
	printf("Parallel IEA: %f segundos", (double)(end - start) / CLOCKS_PER_SEC);
	fflush(stdout);
	//**************************************************
	...
	if(error != 0)
	{
		printf("EXECUTION ERROR IEA Paralle: Error writing header file: %s.", header_filename);
		fflush(stdout);
		exit(-1);
	}
	...
	if(error != 0)
	{
		printf("EXECUTION ERROR IEA Paralle: Error writing image file: %s.", argv[3]);
		fflush(stdout);
		exit(-1);
	}
