...
int main(int argc, char* argv[])
{
	...
	if(argc !=  4)
	{
		printf("EXECUTION ERROR FCLSU Iterative: Parameters are not correct.");
		printf("./FCLSU [Image Filename] [Endmembers file] [Output Result File]");
		fflush(stdout);
		exit(-1);
	}
	...
	if(error != 0)
	{
		printf("EXECUTION ERROR FCLSU Iterative: Error reading header file: %s.", header_filename);
		fflush(stdout);
		exit(-1);
	}
	...
	if(error != 0)
	{
		printf("EXECUTION ERROR FCLSU Iterative: Error reading image file: %s.", argv[1]);
		fflush(stdout);
		exit(-1);
	}
	...
	if(error != 0)
	{
		printf("EXECUTION ERROR FCLSU Iterative: Error reading endmembers header file: %s.", header_filename);
		fflush(stdout);
		return error;
	}
	...
	if(error != 0)
	{
		printf("EXECUTION ERROR FCLSU Iterative: Error reading endmembers file: %s.", argv[2]);
		fflush(stdout);
		return error;
	}
	...
	int i, j, k;
	int bandsExt = bands+1;
	int bandsEndExt = bandsEnd+1;

	double *endmembersExt = (double*)malloc(targets*bandsEndExt*sizeof(double));
	double *imageExt = (double*)malloc(lines*samples*bandsExt*sizeof(double));

	for(i = 0; i<lines*samples; i++)
		for(j=0; j<bandsExt; j++)
			if(j < bands) imageExt[j*lines*samples+i] = image[j*lines*samples+i];
			else imageExt[j*lines*samples+i] = 1;

	for(i = 0; i<targets; i++)
		for(j=0; j<bandsEndExt; j++)
			if(j < bandsEnd) endmembersExt[j*targets+i] = endmembers[j*targets+i];
			else endmembersExt[j*targets+i] = 1;


	double alpha = 1, beta = 0;
	double *Et_E = (double*)malloc(targets*targets*sizeof(double));
	dgemm_("N", "T", &targets, &targets, &bandsEndExt, &alpha, endmembersExt, &targets, endmembersExt, &targets, &beta, Et_E, &targets);
	...
	double *COMPUT = (double*)malloc(targets*bandsEndExt*sizeof(double));
	dgemm_("N", "N", &targets, &bandsEndExt, &targets, &alpha, Et_E, &targets, endmembersExt, &targets, &beta, COMPUT, &targets);

	int lines_samples = lines*samples;
	double *ABUN = (double*)malloc(targets*lines_samples*sizeof(double));

	dgemm_("N","T", &lines_samples, &targets, &bandsExt, &alpha, imageExt, &lines_samples, COMPUT, &targets, &beta, ABUN, &lines_samples);

	//Iterative process*********************************


	bool terminated, all, change;
	int iteration, counter, step, pos, one = 1;
	double max;

	int *P = (int*)calloc(targets,sizeof(int));
	int *R = (int*)calloc(targets,sizeof(int));
	int *S = (int*)calloc(targets,sizeof(int));

	double *FCLSABUN = (double*)calloc(targets,sizeof(double));
	double *LSABUN = (double*)calloc(targets,sizeof(double));
	double *ALFAR = (double*)calloc(targets,sizeof(double));
	double *PHI = (double*)calloc(targets*targets,sizeof(double));
	double *LANDAK = (double*)calloc(targets,sizeof(double));
	double *DELTA = (double*)calloc(targets*targets,sizeof(double));
	double *ALFAS = (double*)calloc(targets,sizeof(double));
	...
	for(i=0; i<lines*samples; i++)
	{
		...
		while(!terminated && iteration < MAXITERATIONS)
		{
			...
			case 2:
				for(j=0; j<targets; j++)
					FCLSABUN[j] = LSABUN[j] = ABUN[i*targets+j];
				step = 3;
				break;
			case 3:
				if(!anyNegative(FCLSABUN, targets))
				{
					for(j=0; j<targets; j++)
						ABUN[i*targets+j] = FCLSABUN[j];
					terminated = true;
				}
				else step = 4;
				break;
			...
			case 13:
				...
				break;
			case 14:
				dgemm_("N", "N", &targets, &one, &targets, &alpha, DELTA, &targets, LANDAK, &targets, &beta, FCLSABUN, &targets);
				for(j=0; j<targets; j++)
					FCLSABUN[j] = LSABUN[j] - FCLSABUN[j];
				step = 3;
				break;
			}
		}
	...
	printf("Iterative FCLSU: %f segundos", (double)(end - start) / CLOCKS_PER_SEC);
	fflush(stdout);
	//**************************************************
	...
	if(error != 0)
	{
		printf("EXECUTION ERROR FCLSU Iterative: Error writing endmembers header file: %s.", header_filename);
		fflush(stdout);
		return error;
	}
	...
	if(error != 0)
	{
		printf("EXECUTION ERROR FCLSU Iterative: Error writing endmembers file: %s.", argv[3]);
		fflush(stdout);
		return error;
	}
	//FREE MEMORY***************************************
